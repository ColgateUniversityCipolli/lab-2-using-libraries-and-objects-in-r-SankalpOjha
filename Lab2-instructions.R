...
for(i in 1:length(album)){
  ...
  for(y in 1:length(track.files)){
    ...
    artist <- str_split_i(song.name.nowav[y], "-", 2)
    album <- str_split_i(full.file.location[y], "/", 3)
    track <- str_split_i(song.name.nowav[y], "-", 3)
...
#                               end = (length(songnames[i])-6)))
 ...
 # for (j in 1:length(list.files(path = file.paths[album[i]]))){
  #j=1
    #Step C
